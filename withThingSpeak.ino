...
#define THINGSPEAK_API_KEY "11ISV1M2E9CZS4WP"
#define THINGSPEAK_CHANNEL_ID 2960418
...
// Constants for Water Level Monitoring (Parameter Galon)
const float EMPTY_GALON_DISTANCE = 34.0;
const float FULL_GALON_DISTANCE = 9.0;
const float GALON_TINGGI = 34.0;
const float GALON_DIAMETER = 15.0;
const float GALON_RADIUS = GALON_DIAMETER / 2.0;
const float GALON_LUAS_ALAS = PI * GALON_RADIUS * GALON_RADIUS;
...
void setup() {
  ...
  // Set timer to send water level data to Blynk and ThingSpeak every 1 detik
...
void sendWaterLevelData() {
  ...
  float waterVolume = GALON_LUAS_ALAS * waterHeight / 1000.0;
  waterVolume = constrain(waterVolume, 0, (GALON_LUAS_ALAS * GALON_TINGGI) / 1000.0);
  ...
  ThingSpeak.writeField(THINGSPEAK_CHANNEL_ID, 1, waterHeight, THINGSPEAK_API_KEY); // Field 1: Tinggi Air
  ThingSpeak.writeField(THINGSPEAK_CHANNEL_ID, 2, waterVolume, THINGSPEAK_API_KEY); // Field 2: Volume Air
}
